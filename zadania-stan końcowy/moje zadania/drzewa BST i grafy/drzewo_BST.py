...
class BSTree:
    ...
    def wstawianie_iteracyjne(self, klucz):
        """
        Wstawia klucz do drzewa w sposób iteracyjny.

        :param klucz: Wartość klucza do wstawienia do drzewa.
        """
        nowy_wezel = Wezel(klucz)
        ...
        obecny = self.korzen
        while True:
            if klucz < obecny.klucz:
                if obecny.lewy is None:
                    obecny.lewy = nowy_wezel
                    return
                obecny = obecny.lewy
            elif klucz > obecny.klucz:
                if obecny.prawy is None:
                    obecny.prawy = nowy_wezel
                    return
                obecny = obecny.prawy
            else:
                # Klucz już istnieje, nic nie rób
                return
    ...
    def czy_zrownowazone(self):
        def _czy_zrownowazone(wezel):
            if wezel is None:
                return True, 0

            lewa_zrownowazona, lewa_wysokosc = _czy_zrownowazone(wezel.lewy)
            prawa_zrownowazona, prawa_wysokosc = _czy_zrownowazone(wezel.prawy)

            zrownowazona = (
                lewa_zrownowazona and prawa_zrownowazona and abs(lewa_wysokosc - prawa_wysokosc) <= 1
            )

            return zrownowazona, max(lewa_wysokosc, prawa_wysokosc) + 1

        zrownowazona, _ = _czy_zrownowazone(self.korzen)
        return zrownowazona

    def usun_klucz(self, klucz):
        def _usun(wezel, klucz):
            if wezel is None:
                return wezel

            if klucz < wezel.klucz:
                wezel.lewy = _usun(wezel.lewy, klucz)
            elif klucz > wezel.klucz:
                wezel.prawy = _usun(wezel.prawy, klucz)
            else:
                if wezel.lewy is None:
                    return wezel.prawy
                ...
                min_w_prawym = self._min_klucz(wezel.prawy)
                wezel.klucz = min_w_prawym
                wezel.prawy = _usun(wezel.prawy, min_w_prawym)

            return wezel

        self.korzen = _usun(self.korzen, klucz)

    def wezly_na_poziomie(self, poziom):
        def _wezly_na_poziomie(wezel, aktualny_poziom):
            if wezel is None:
                return 0
            ...
            return _wezly_na_poziomie(wezel.lewy, aktualny_poziom - 1) + _wezly_na_poziomie(wezel.prawy, aktualny_poziom - 1)

        return _wezly_na_poziomie(self.korzen, poziom)
...
if __name__ == "__main__":
    ...
    drzewo._LRV(drzewo.korzen)

    print("\nPre-order (VLR):")
    drzewo._VLR(drzewo.korzen)

    print("\nIn-order (LVR):")
    drzewo._LVR(drzewo.korzen)
