...
def sortowanie_alfabetyczne(lista):
    """
    Sortowanie listy stringów alfabetycznie przy użyciu sortowania bąbelkowego.

    :param lista: Lista stringów do posortowania.
    :return: Posortowana lista stringów.
    """
    n = len(lista)
    for i in range(n):
        for j in range(0, n - i - 1):
            if lista[j] > lista[j + 1]:  # Porównanie alfabetyczne stringów
                lista[j], lista[j + 1] = lista[j + 1], lista[j]  # Zamiana
    return lista
...
sortuj_alfabetycznie = sortowanie_alfabetyczne(nazwiska)
print("\n", sortuj_alfabetycznie)
